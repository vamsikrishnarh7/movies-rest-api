...
try:
    ...
    if not os.path.isdir(test_functional_dir):
        test_functional_dir = os.path.join(
            os.getenv("HOME", "/home/travis"), "pylint", "tests"
        )
...
def get_tests(input_dir="input", sort=False):
    ...
    for fname in input_dir.iterdir():
        if fname.name != "__init__.py" and fname.name.endswith(".py"):
            suite.append(
                FunctionalTestFile(str(input_dir.absolute()), str(fname.absolute()))
            )
...
@pytest.mark.parametrize(
    "test_file", MIGRATIONS_TESTS[:1], ids=MIGRATIONS_TESTS_NAMES[:1]
)
